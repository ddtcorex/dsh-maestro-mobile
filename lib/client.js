    ...
    const mark = () => {
        ...
        for (const root of document.querySelectorAll('[data-phase] [class*="_root"]')) {
            ...
            if (!/(turns|steps|\bLLM\b|轮|步)/.test(text))
                continue;
            if (root.querySelector('[data-composer-input]') !== null)
                continue;
...
__modules["effects/composer-keyboard-touch.js"] = function (require, module, exports) {
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.shouldSuppressComposerMousedown = shouldSuppressComposerMousedown;
exports.installComposerKeyboardTouch = installComposerKeyboardTouch;
const phone_chrome_ts_1 = require("./effects/phone-chrome.js");
/**
 ...
 */
function shouldSuppressComposerMousedown(target) {
    ...
    if (button.closest(COMPOSER_CARD_SELECTOR) === null)
        return false;
    // The @ candidate menu picks on mousedown — suppressing it breaks touch
    // selection, so its rows and crumbs pass through untouched.
    if (button.closest(TRIGGER_MENU_SELECTOR) !== null)
        return false;
    return true;
}
function installComposerKeyboardTouch(ctx) {
    (0, phone_chrome_ts_1.installMobileEffect)(ctx, 'dsh-maestro-mobile: composer keyboard touch guard', () => {
        if (typeof PointerEvent === 'undefined' && typeof TouchEvent === 'undefined')
            return undefined;
        let touchUntil = 0;
        const onTouchActivity = () => {
        ...
        const onMouseDown = (event) => {
            if (Date.now() > touchUntil)
                return;
            const target = event.target;
            if (!(target instanceof Element))
                return;
            if (!shouldSuppressComposerMousedown(target))
                return;
...
const composer_keyboard_touch_ts_1 = require("./effects/composer-keyboard-touch.js");
...
function apply(ctx) {
    ...
    (0, composer_keyboard_touch_ts_1.installComposerKeyboardTouch)(ctx);
