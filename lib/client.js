...
/** dsh-web-ui compatibility: explorer / preview column visibility markers and rise animation (same origin, same mechanism, merged). */
...
function createStatsLineTask() {
    ...
    const mark = () => {
        for (const root of document.querySelectorAll('[data-phase] [class*="_root"]')) {
            ...
            if (!/(turns|steps|\bLLM\b)/.test(text))
                continue;
...
__modules["effects/drawer-navigation.js"] = function (require, module, exports) {
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.drawerCloseTiming = drawerCloseTiming;
/**
 ...
 */
function drawerCloseTiming(eventType, isSessionRow) {
    if (!isSessionRow)
        return 'immediate';
...
const drawer_navigation_ts_1 = require("./effects/drawer-navigation.js");
...
function installOverlayInteractions(ctx) {
    installMobileEffect(ctx, 'dsh-maestro-mobile: drawer close (Escape + navigate)', () => {
        ...
        // iOS may run a frame queued from pointerup before it dispatches the
        // synthesized click. Session rows therefore remain mounted through
        // pointerup and close only after their click has propagated to React.
        const isSessionRowTarget = (target) => target instanceof Element && target.closest('[class*="sessionRow"]') !== null;
        let drawerCloseTimer;
        const closeDrawerAfterClick = () => {
        ...
        const closeDrawerForEvent = (eventType, target) => {
            switch ((0, drawer_navigation_ts_1.drawerCloseTiming)(eventType, isSessionRowTarget(target))) {
                case 'ignore': return;
                case 'immediate': return closeDrawerForNavigation();
                case 'after-click': return closeDrawerAfterClick();
            }
        };
        const onDrawerClick = (event) => {
            if (!shouldCloseOnTapInsideDrawer(event.target))
                return;
            closeDrawerForEvent('click', event.target);
        };
        ...
        const onDrawerPointerUp = (event) => {
            ...
            if (!shouldCloseOnTapInsideDrawer(event.target))
                return;
            closeDrawerForEvent('pointerup', event.target);
        };
...
/* ---------- Plugins settings: card list scroll pad ----------
   ul._cards has native padding 0, so scrolled to the bottom the last
   plugin-config row sits flush against the options-area edge and reads cut
   off. Anchored to the stable upstream slot marker (not a class hash) and
   scoped to the mobile frame. */
[data-mobile-nav="frame"] [aria-modal="true"] [data-slot="settings.plugins.tab"] [class*="_cards"] {
  padding-bottom: 20px;
}
}
...
const phone_chrome_ts_1 = require("./effects/phone-chrome.js");
/**
 * Touch support for the lineage-count chip ("N subagents") that
 ...
 *   armed open timer, so the card never appears ("no response after tapping").
