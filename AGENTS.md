...
- **Identify an upstream trigger by what it declares or contains, never by being the first of its kind in a slot.** Upstream extends slots between releases (0.1.7 moved the subagent catalog into the session header's actions band ahead of the jobs control), so a first-match rule silently marks the wrong control. The jobs chip is identified by the popup roles its neighbours declare plus its own leading count badge or state dot, and the task refuses an ambiguous band; `pnpm probe:jobs-chip` gates it. The header band paints one 8px gap across its residents — no per-resident gutter, and never let it shrink below its content.
- **A scope that must survive upstream rebuilds follows a marker this plugin sets or a role upstream draws unconditionally — never a CSS-module hash.** The lineage chip's synthetic-hover swallow was scoped with the hashes of the day and matched nothing after 0.1.7 rebuilt the package, so it silently stopped firing; the chip's box now carries `data-lineage-root` and its portalled catalog is named by `role="tree"`. `pnpm probe:lineage-chip` gates mark/scope/swallow/toggle, and `tests/no-hashed-class-prefix.test.ts` fails the build on a hash prefix anywhere in `src/client`.
...
- It inherits the deployment gates (Maestro PIN proxy on `:3080`/`:3081` with an `HttpOnly; SameSite=Lax` cookie; loopback bind + launch-token fence on `:3082`) and adds `delete-route.ts` defence in depth — POST only, reject `Sec-Fetch-Site: cross-site`, require an `Origin` whose host matches the request `Host` when one is present, treat a malformed `Origin` as hostile, and accept exactly one non-empty string `sessionId` in a byte-capped JSON body (64 KiB; an oversized body is drained and answered 413 `payload-too-large`, never destroyed mid-stream — destroying races the response).
- **Deletion is recoverable, not destructive.** The session directory moves into `<root>/.sessions-trash/` with every canonical payload renamed to `<name>.trash` plus a best-effort `manifest.json`; restoring is "strip the suffix". The rename MUST precede the directory move: the host scans every directory under the storage root, trash included, and a canonical payload name there breaks the whole session list (`docs/maintenance/pitfalls.md`). Entries expire after 24h, purged inline on a later deletion — never add a timer for it.
- **A failure after a LIVE session was torn down reports `cleanup-failed` with `deletedLiveSession: true`** and still settles the workspace accounting: the session is already stopped and unregistered, so a plain `delete-failed` would both mislead and leave a zombie id.
